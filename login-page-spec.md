...
- **Greeting only** (user decision): a heading like `Welcome, tarik!` (username injected from the session flag) plus a **Logout** button.
- No profile card, no session metadata, no timestamps.
...
- A **Logout button** exists (user decision).
...
1. **Happy path:** enter `tarik` + `123456` → welcome page shows "Welcome, tarik!".
...
- [ ] Labels, `aria-invalid`, `aria-describedby`/`role="alert"` are present and correct.
- [ ] Pages render correctly at mobile widths (no overflow at 320px).
